...
const INITIAL_CONDITION = :gaussian # :gaussian, :curl_free or :zero, NOT relevant if USE_MMS=true
...
const y_l = -2.0
const y_r = 2.0
const mx = 40                     # grid points in x
const my = 40                     # grid points in y
...
const DESC_STRING = "CUDA_$(USE_CUDA)_M-S=$(M_ORDER)-$(S_ORDER)_mu$(μ_tag)_t0_$(replace(string(T0), "." => "_"))_SIGMA_$(replace(string(SIGMA), "." => "_"))_SCHEMA_$(SCHEMA)_$(PERIODIC ? "periodic" : "non_periodic")_IC_$(INITIAL_CONDITION)_order_$(SBP_ORDER)_mxmy_$(mx)x$(my)"
