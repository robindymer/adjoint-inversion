...
function [pars, truePars] = lambdaElAc2Blocks(opts)
...
% Perturb mu
perturbedLambda = {@(x,y) 2*lambda(x,y), @(x,y) 1.5*lambda(x,y)} ;
...
pars = struct;
pars.domain = domain;
pars.elastic.lambda = perturbedLambda;
pars.elastic.mu = mu;
...
truePars = pars;
truePars.elastic.lambda = lambda;
