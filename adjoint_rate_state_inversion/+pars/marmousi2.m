...
function [pars, truePars] = marmousi2(opts, downSampling)

% Default values
default_arg('downSampling', 20);
default_arg('opts', struct);

opts = defaultField(opts, 'nRecEl', 10);
opts = defaultField(opts, 'nRecAc', 10);
...
x = linspace(xl,xr,nS);
D = 10;
y = linspace(ymax_w-D,ymax_w-D,nS);
...
%--- Elastic receivers ---
x = linspace(xl,xr,nRecEl);
if nRecEl == 1
	x = (xl+xr)/2;
end
y = linspace(ymax_e,ymax_e,nRecEl);
receivers.elastic.x = cell(1, nRecEl);
for i = 1:nRecEl
...
%--- Acoustic receivers ---
x = linspace(xl,xr,nRecAc);
y = linspace(ymin_w,ymin_w,nRecAc);
receivers.acoustic.x = cell(1, nRecAc);
for i = 1:nRecAc
...
truePars = pars;
truePars.elastic.rho = perturbedRho;
