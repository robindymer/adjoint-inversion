...
classdef Elastic2DOpt < adjopt.AdjointOptimization
	...
	methods
		...
		function obj = Elastic2DOpt(pars, inversionPars, order, k)
			...
			forwardDiscr = elastic.elasticDiscr(m, order, xlim, ylim, lambda, mu, rho,...
                                               [], bc, sources);
			obj.dim = forwardDiscr.diffOp.dim;
			...
			if ismember('lambda', ip) || ismember('mu',ip)
				...
				mu_dummy = 0*mu;
				toyDiscr = elastic.elasticDiscr(m, order, xlim, ylim, lambda_dummy, mu_dummy, rho,...
                                               [], bc, sources);
		...
		function runForward(obj, plotFlag, T)
			...
			[receiverData, waveField] = obj.runSimulation(discr, T, receiverPoints, receiverDeltas, plotFlag);
			obj.forwardReceiverRecordings = receiverData;
			obj.forwardField = waveField;
		end

		function runAdjoint(obj, plotFlag, T)
			default_arg('plotFlag', false);
			default_arg('T', obj.T);
		...
		function [receiverRecordings, waveField] = runSimulation(obj, discr, T, receiverPoints, receiverDeltas, plotFlag)
			...
			% No movie
				...
				% fprintf('Running PDE solve o = %d, mx = %d.  \n', obj.order, obj.m(1));
				for i = 1:N
					ts.step();
					[~, ~, V, ~, Vt] = ts.getV;
					ii = (i-1)*nStages+1 : i*nStages;
		...
		function discr = createElasticDiscr(obj, pars)
			...
			discr = elastic.elasticDiscr(m, order, xlim, ylim, lambda, mu, rho,...
                                               [], bc, sources);
		...
		function grad = gradientFormula(obj)
			...
			for i = 1:numel(obj.inversionPars)
				...
				if ismember(obj.inversionPars{i}, {'lambda','mu'})
					...
					for b = 1:length(obj.pars.bc.names)
						boundary = obj.pars.bc.names{b};
						comp = obj.pars.bc.types{b}{1};
						type = obj.pars.bc.types{b}{2};

						if ismember(type, {'D','d','dirichlet','Dirichlet'})

							switch obj.inversionPars{i}
							case 'mu'
								tau 	= obj.diffOp_mu_1.getBoundaryOperator(['tau' num2str(comp)], boundary);
								alpha_c = obj.diffOp_mu_1.getBoundaryOperator(['alpha' num2str(comp)], boundary);
							case 'lambda'
								tau 	= obj.diffOp_lambda_1.getBoundaryOperator(['tau' num2str(comp)], boundary);
								alpha_c = obj.diffOp_lambda_1.getBoundaryOperator(['alpha' num2str(comp)], boundary);
							end
							e_c = obj.forwardDiscr.diffOp.getBoundaryOperator(['e' num2str(comp)], boundary);
							H_b = obj.forwardDiscr.diffOp.getBoundaryQuadratureForScalarField(boundary);
							tuning = 1.2;

							grad_temp = (e_c'*psi) .* (H_b*tau'*u);
							grad_temp = grad_temp + (tau'*psi) .* (H_b*e_c'*u);
							grad_temp = grad_temp - (e_c'*psi) .* (H_b*tuning*alpha_c'*u);

							% Integrate in time
							grad_temp = obj.timeIntegration(grad_temp);

							% Project and add to gradient
							e = obj.forwardDiscr.diffOp.getBoundaryOperatorForScalarField('e', boundary);
							grad{i} = grad{i} + e * grad_temp;
						end
					end
				end
			end
		...
		function plotParameterComparison(obj, parName, parsets, fontsize)
			...
			case {'rho','lambda','mu'}
				...
				dpar = grid.funcToPlotMatrix(g, dpar);
				X = g.matrices();
				surf(X{1}', X{2}', dpar);
				view(0,90);
				shading interp;
		...
		function [upPar, upRec, upMis, figure_handle] = setupPlot(obj, trueParset, initialParset, M, M0, iter)
			...
			obj.plotParameterComparison(obj.inversionPars{1} ,{trueParset, obj.pars, initialParset}, fontsize);
			...
			function updateParameter(object)
				axes(parfig);
				object.plotParameterComparison(obj.inversionPars{1} ,{trueParset, object.pars, initialParset}, fontsize);
			end
		...
		function plotGradient(obj, grad)
			...
			case {'rho','lambda','mu'}
				...
				grad_mat = grid.funcToPlotMatrix(g, grad{1});
				X = g.matrices();
				surf(X{1}', X{2}', grad_mat);
				view(0,90);
				shading interp;
				colorbar
