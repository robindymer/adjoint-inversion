...
function g = boundaryOptimizedCurvilinear(mapping, m, varargin)
    ...
    if length(varargin) == n+2 % Stencil options supplied
        availabe_opts = ["Accurate","accurate","A","acc","Minimal","minimal","M","min","accurate-upwind"];
        assert(any(varargin{n+2} == availabe_opts), ...
            'grid:boundaryOptimizedCurvilinear:InvalidOption',"The operator option must be 'accurate', 'minimal', or accurate-upwind");
    ...
    switch varargin{n+2}
        ...
        case {'accurate-upwind'}
            gridgenerator = @sbp.grid.upwindAccurateBoundaryOptimizedGrid;
    end
