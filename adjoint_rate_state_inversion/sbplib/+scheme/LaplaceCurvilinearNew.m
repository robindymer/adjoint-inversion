classdef LaplaceCurvilinearNew < scheme.Scheme
    properties
        ...
        n_w, n_e, n_s, n_n % Boundary normals
        t_w, t_e, t_s, t_n % Boundary tangents
    ...
    methods
        ...
        function obj = LaplaceCurvilinearNew(g, order, a, b, opSet)
            ...
            K{2,2} = x_u./J;
            obj.K = cell(dim, dim);
            obj.K{1,1} = spdiag(K{1,1});
            obj.K{1,2} = spdiag(K{1,2});
            obj.K{2,1} = spdiag(K{2,1});
            obj.K{2,2} = spdiag(K{2,2});
        ...
        function [closure, penalty] = boundary_condition(obj, boundary, type, parameter)
            ...
            switch type
                ...
                % Traction bc. Similar to Neumann, but note that the penalty is for b*du/dn.
                case {'T','t','traction'}
                    tau1 = -1;
                    tau2 = 0;
                    tau = (tau1*e + tau2*d)*H_b;
        ...
        function o = getBoundaryOperator(obj, op, boundary)
            assertIsMember(op, {'e', 'd', 'traction'})
        ...
        % Returns square matrix of the specified field, of dimension
        % corresponding to the number of boundary points
        %
        % boundary -- string
        function M = getBoundaryField(obj, field, boundary)
            assertIsMember(boundary, {'w', 'e', 's', 'n'})
            assertIsMember(field, {'gamma'})

            switch field
            case 'gamma'
                % Symmetric term in Dirichlet penalty, only the "penalty" part, not closure.
                e               = obj.getBoundaryOperator('e', boundary);
                H_b             = obj.getBoundaryQuadrature(boundary);
                s_b             = obj.getBoundaryScaling(boundary);
                [th_H, ~, th_R] = obj.getBoundaryBorrowing(boundary);
