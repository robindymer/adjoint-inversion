classdef DiffOp < scheme.Scheme
    ...
    methods
        ...
        function field = getBoundaryField(obj, fieldName, boundary)
            switch class(boundary)
                case 'cell'
                    blockId = boundary{1};
                    field = obj.diffOps{blockId}.getBoundaryField(fieldName, boundary{2});
                    return
                case 'multiblock.BoundaryGroup'
                    N = length(boundary);
                    H_bm = cell(N,N);
                    for i = 1:N
                        H_bm{i,i} = obj.getBoundaryField(fieldName, boundary{i});
                    end
                    field = blockmatrix.toMatrix(H_bm);
                otherwise
                    error('Unknown boundary indentifier')
            end
        end
