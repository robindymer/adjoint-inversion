classdef elasticAnisotropicStaggeredSupergridDiscr < noname.Discretization
    properties
        name         = 'Staggered Anisotropic elastic 2D multiblock curvilinear with supergrid'
        description  = 'rho*u_{j,tt} = d_i C_ijkl d_k u_l + F(x,y,t)'
        order        %Order of accuracy
        dim          %Number of spatial dimensions

        D            %Discretization matrix including BC
        E            %Matrix for u_t
        H            %Total quadrature, for both components

        % v_tt = D*v + E*v_t + S_cont(t) + S_discr
        S_cont       %Function handle, v_tt = D*v + E*v_t + S_cont(t) + S_discr
        S_discr      %Matrix of time-dependent data, one column per time-stepping stage.

        v0           %Initial data
        v0t          %Initial data for time derivative
        diffOp       %Various operators
        grid         %Multiblock grid in general

        %RHO          %Density, diagonal matrix
        RHO_kron     %Kroneckered density
        %C            %Elastic stiffness tensor
        %sigma        %Elastic stress operator

        Ecomp        % Ecomp{1}' picks out first component, Ecomp{2}' picks out 2nd comp.
        superGrid
        stencil

        U
        G
    end

    methods

        % bc should be a cell array of structs, one struct per BC.
        % Each struct contains the fields
        %       - boundary: e.g. {1, 'w'} for west boundary, 1st block
        %       - type:     e.g. {2, 'd'} for displacement condition on the 2nd component
        %       - data:     boundary data function handle
        function obj = elasticAnisotropicStaggeredSupergridDiscr(g, order, C, rho, F,...
                                                            bc, pointSources, superGrid)
            default_arg('pointSources',[]);
            default_arg('F',[]);
            default_arg('bc',[]);
            default_arg('rho',[]);
            default_arg('C',[]);
            default_arg('order',4);

            nGrids = 2;
            dim = 2;
            obj.dim = dim;

            % Domain definition, grid, diffOp
            obj.grid = g;

            nBlocks = g.nBlocks();
            doParam = cell(nBlocks, 1);

            for i = 1:nBlocks
                if iscell(rho)
                    doParam{i} = {rho{i}, C{i}};
                elseif isa(rho, 'function_handle')
                    doParam{i} = {rho, C};
                else
                    error('Inconsistent format for C and rho.');
                end
            end

            diffOp = multiblock.DiffOp(@scheme.Elastic2dStaggeredCurvilinearAnisotropic, g, order, doParam);

            D = diffOp.D;
            H = diffOp.H;

            RHO_kron = cell(nBlocks, nBlocks);

            U11 = cell(nBlocks, nBlocks);
            U12 = cell(nBlocks, nBlocks);
            U21 = cell(nBlocks, nBlocks);
            U22 = cell(nBlocks, nBlocks);
            G1 = cell(nBlocks, nBlocks);
            G2 = cell(nBlocks, nBlocks);

            Ec1 = cell(nBlocks, nBlocks);
            Ec2 = cell(nBlocks, nBlocks);

            I_dim = speye(2,2);
            for b = 1:nBlocks
                RHO1 = diffOp.diffOps{b}.RHO{1};
                RHO2 = diffOp.diffOps{b}.RHO{2};

                RHO1 = kron(RHO1, I_dim);
                RHO2 = kron(RHO2, I_dim);

                RHO_temp = {RHO1, [];...
                            [], RHO2};
                RHO_kron{b,b} = blockmatrix.toMatrix(RHO_temp);

                G1{b,b} = diffOp.diffOps{b}.G{1};
                G2{b,b} = diffOp.diffOps{b}.G{2};
                U11{b,b} = diffOp.diffOps{b}.U{1}{1};
                U21{b,b} = diffOp.diffOps{b}.U{2}{1};
                U12{b,b} = diffOp.diffOps{b}.U{1}{2};
                U22{b,b} = diffOp.diffOps{b}.U{2}{2};

                E11 = (U11{b,b}'*G1{b,b}')';
                E21 = (U21{b,b}'*G2{b,b}')';
                Ec1{b,b} = [E11, E21];

                E12 = (U12{b,b}'*G1{b,b}')';
                E22 = (U22{b,b}'*G2{b,b}')';
                Ec2{b,b} = [E12, E22];
            end
            RHO_kron = blockmatrix.toMatrix(RHO_kron);
            obj.RHO_kron = RHO_kron;

            G1 = blockmatrix.toMatrix(G1);
            G2 = blockmatrix.toMatrix(G2);
            obj.G = {G1, G2};

            U11 = blockmatrix.toMatrix(U11);
            U12 = blockmatrix.toMatrix(U12);
            U21 = blockmatrix.toMatrix(U21);
            U22 = blockmatrix.toMatrix(U22);

            obj.U = cell(nGrids, 1);
            for a = 1:nGrids
                obj.U{a} = cell(dim, 1);
            end
            obj.U{1}{1} = U11;
            obj.U{2}{1} = U21;
            obj.U{1}{2} = U12;
            obj.U{2}{2} = U22;

            Ec1 = blockmatrix.toMatrix(Ec1);
            Ec2 = blockmatrix.toMatrix(Ec2);
            Ec = {Ec1, Ec2};
            obj.Ecomp = Ec;

            %---- Forcing function F ----
            % Assume same forcing everywhere

            % Evaluate F
            if ~isempty(F)
                F_comb = @(t,x,y) [F{1}(t,x,y); F{2}(t,x,y)];
                Ft = @(t) RHO_kron\grid.evalOnStaggered(g, @(x,y)F_comb(t,x,y));
            else
                Ft = [];
            end
            %---------------------------

            % Set BC
            [closure, penalty] = scheme.bcSetupStaggered(diffOp, bc);
            D = D + closure;

            %----- Point sources ----------------
            discrSource = false;
            contSource = false;
            if ~isempty(pointSources)

                x_s = pointSources.x;
                blockIds = pointSources.blockIds;
                n_s = length(x_s);

                % Initialize
                source_func_cont = @(t) 0*t;
                source_func_discr = 0;

                for s = 1:n_s

                    % Check which grid block source is in
                    blockId = blockIds(s);

                    % Delta functions corresponding to local grid
                    deltas = elastic.diracDiscrCurve(x_s{s}, g.grids{blockId}, order, 0, order, 'staggered');

                    % Expand to local gridGroup
                    delta_local = [deltas.d1; deltas.d2];

                    % Extend delta fun to global grid.
                    d = g.expandFunc(delta_local, blockId);

                    % Loop over components
                    for i = 1:dim

                        % Project source for component i to full grid size and accumulate different sources.
                        if isa(pointSources.g{s}{i}, 'function_handle')
                            source_func_cont = @(t) source_func_cont(t) + ...
                                               RHO_kron \ (pointSources.g{s}{i}(t)*Ec{i}*d);
                            contSource = true;
                        else
                            source_vec = pointSources.g{s}{i};
                            if iscolumn(source_vec)
                                source_vec = transpose(source_vec);
                            end
                            source_func_discr = source_func_discr + ...
                                               RHO_kron \ kron(source_vec, Ec{i}*d);

                            discrSource = true;
                        end
                    end
                end
            end

            if discrSource
                ps_discr = source_func_discr;
            else
                ps_discr = [];
            end

            if contSource
                ps_cont = source_func_cont;
            else
                ps_cont = [];
            end
            % -----------------------------------

            % Create data function S(t)
            data_funcs = {Ft, penalty, ps_cont};
            S = elastic.addFunctionHandles(data_funcs);
            obj.S_cont = S;
            obj.S_discr = ps_discr;

            % Zero initial data, two components of displacement
            v0_fun = @(x,y) [0*x; 0*y];
            v0t_fun = @(x,y) [0*x; 0*y];

            obj.v0 = grid.evalOnStaggered(g, v0_fun);
            obj.v0t = grid.evalOnStaggered(g, v0t_fun);

            % Misc
            diffOp.D = D;
            obj.diffOp = diffOp;
            obj.D = D;
            obj.H = H;
            obj.order = order;

            % ------- Supergrid ------- %
            defaultSuperGrid.gamma = {0, 5e-3, 0};
            defaultSuperGrid.DOI_IDs = 1:g.nBlocks();
            defaultSuperGrid.W_IDs = [];
            defaultSuperGrid.E_IDs = [];
            defaultSuperGrid.S_IDs = [];
            defaultSuperGrid.N_IDs = [];
            default_struct('superGrid', defaultSuperGrid);
            [gamma, DOI_IDs] = dealStruct(superGrid, {'gamma', 'DOI_IDs'});

            if gamma{1} ~= 0
                error('gamma_1 ~= 0 not yet implemented');
            end

            %--- Estimate max wave speed ----
            % Compute speed in a few different directions
            v_max = zeros(nBlocks, 1);
            for b = 1:nBlocks

                % Get transformed stiffness tensor
                C = obj.diffOp.diffOps{b}.refObj.C{1};
                for i = 1:numel(C)
                    C{i} = diag(C{i});
                end

                % Get transformed density evaluated on stress grid
                rho = obj.interpolateRhoToStressGrid(b);

                % Compute speed in a few different directions
                theta_range = linspace(0,pi,5);
                v_max(b) = elastic.maxAnisotropicWaveSpeed(rho, C, theta_range);
            end
            v = max(v_max);

            % Build operators
            D_empty = cell(nGrids, 1);
            for a = 1:nGrids
                D_empty{a} = cell(nBlocks, nBlocks);
            end

            D2xi = D_empty;
            D2eta = D_empty;
            D4xi = D_empty;
            D4eta = D_empty;
            Hxi = D_empty;
            Heta = D_empty;

            for i = 1:nBlocks

                % Get reference grid size
                gi = g.grids{i};
                mi = gi.gridGroups{1}{1}.size();

                hxi = 1/(mi(1) - 1);
                heta = 1/(mi(2) - 1);

                ops = cell(dim,1);
                D1p = cell(dim, 1);
                D1d = cell(dim, 1);
                D2p = cell(dim, 1);
                D2d = cell(dim, 1);
                mp = cell(dim, 1);
                md = cell(dim, 1);
                Ip = cell(dim, 1);
                Id = cell(dim, 1);

                opSet = @sbp.D1StaggeredUpwind;
                for j = 1:dim
                    ops{j} = opSet(mi(j), {0,1}, order);
                    D1p{j} = ops{j}.D1_dual;
                    D1d{j} = ops{j}.D1_primal;
                    D2p{j} = D1d{j}*D1p{j};
                    D2d{j} = D1p{j}*D1d{j};
                    mp{j} = length(ops{j}.x_primal);
                    md{j} = length(ops{j}.x_dual);
                    Ip{j} = speye(mp{j}, mp{j});
                    Id{j} = speye(md{j}, md{j});
                end

                flag = ~ismember(i, DOI_IDs);
                D2xi{1}{i,i} = flag*kron(D2p{1}, Ip{2});
                D2xi{2}{i,i} = flag*kron(D2d{1}, Id{2});
                D2eta{1}{i,i} = flag*kron(Ip{1}, D2p{2});
                D2eta{2}{i,i} = flag*kron(Id{1}, D2d{2});

                for b = 1:nGrids
                    D4xi{b}{i,i} = D2xi{b}{i,i}*D2xi{b}{i,i};
                    D4eta{b}{i,i} = D2eta{b}{i,i}*D2eta{b}{i,i};
                end
            end

            % Compute "reference Jacobian", i.e value of Jacobian in DOI
            Jref = obj.diffOp.diffOps{DOI_IDs(1)}.J;
            Jref = mean(diag(Jref));

            for a = 1:nGrids
                D2xi{a} = blockmatrix.toMatrix(D2xi{a});
                D2eta{a} = blockmatrix.toMatrix(D2eta{a});
                D4xi{a} = blockmatrix.toMatrix(D4xi{a});
                D4eta{a} = blockmatrix.toMatrix(D4eta{a});
            end

            G = obj.G;
            U = obj.U;

            D2xi = G{1}*U{1}{1}*D2xi{1}*U{1}{1}'*G{1}' + G{2}*U{2}{1}*D2xi{2}*U{2}{1}'*G{2}' + ...
                   G{1}*U{1}{2}*D2xi{1}*U{1}{2}'*G{1}' + G{2}*U{2}{2}*D2xi{2}*U{2}{2}'*G{2}';

            D2eta = G{1}*U{1}{1}*D2eta{1}*U{1}{1}'*G{1}' + G{2}*U{2}{1}*D2eta{2}*U{2}{1}'*G{2}' + ...
                   G{1}*U{1}{2}*D2eta{1}*U{1}{2}'*G{1}' + G{2}*U{2}{2}*D2eta{2}*U{2}{2}'*G{2}';

            D4xi = G{1}*U{1}{1}*D4xi{1}*U{1}{1}'*G{1}' + G{2}*U{2}{1}*D4xi{2}*U{2}{1}'*G{2}' + ...
                   G{1}*U{1}{2}*D4xi{1}*U{1}{2}'*G{1}' + G{2}*U{2}{2}*D4xi{2}*U{2}{2}'*G{2}';

            D4eta = G{1}*U{1}{1}*D4eta{1}*U{1}{1}'*G{1}' + G{2}*U{2}{1}*D4eta{2}*U{2}{1}'*G{2}' + ...
                   G{1}*U{1}{2}*D4eta{1}*U{1}{2}'*G{1}' + G{2}*U{2}{2}*D4eta{2}*U{2}{2}'*G{2}';

            H = diffOp.H;

            E = ...
            - gamma{2} * Jref^(-1/dim) * (hxi^(4-1) * (H \ (D2xi' * H * v * D2xi))) ...
            - gamma{3} * Jref^(-1/dim) * (hxi^(8-1) * (H \ (D4xi' * H * v * D4xi))) ...
            - gamma{2} * Jref^(-1/dim) * (heta^(4-1) * (H \ (D2eta' * H * v * D2eta))) ...
            - gamma{3} * Jref^(-1/dim) * (heta^(8-1) * (H \ (D4eta' * H * v * D4eta))) ;
            %------------------------------------------ %

            obj.E = E;
            obj.superGrid = superGrid;

        end
        % Prints some info about the discretisation
        function printInfo(obj)
            fprintf('Name: %s\n',obj.name);
            fprintf('Size: %d\n',obj.size());
        end

        % Return the number of DOF
        function n = size(obj)
            n = length(obj.v0);
        end

        % Returns a timestepper for integrating the discretisation in time
        %     method is a string that states which timestepping method should be used.
        %          The implementation should switch on the string and deliver
        %          the appropriate timestepper. It should also provide a default value.
        %     time_align is a time that the timesteps should align with so that for some
        %                integer number of timesteps we end up exactly on time_align
        function [ts, N] = getTimestepper(obj, method, time_align, k)
            default_arg('method','rk4');
            default_arg('time_align',[]);
            default_arg('k',[]);
            switch method
                case 'rk4'
                    if isempty(k)
                        k = obj.getTimestep(method);
                    end

                    if ~isempty(time_align)
                        [k, N] = alignedTimestep(k, time_align);
                    end

                    t = 0;
                    ts = time.ExplicitRungeKuttaSecondOrderDiscreteData(...
                                            obj.D, obj.E, obj.S_cont, obj.S_discr, k, t, obj.v0, obj.v0t);
                otherwise
                    error('Timestepping method ''%s'' not supported',method);
            end
        end

        function k = getTimestep(obj, method, cfl)
            default_arg('cfl',[]);
            if isempty(cfl)
                switch obj.order
                case 2
                    cfl = 0.5;
                case 4
                    cfl = 0.5;
                case 6
                    cfl = 0.5;
                end
            end

            nBlocks = obj.grid.nBlocks();
            dt = zeros(nBlocks, 1);

            for b = 1:nBlocks

                % Get transformed stiffness tensor
                C = obj.diffOp.diffOps{b}.refObj.C{1};
                for i = 1:numel(C)
                    C{i} = diag(C{i});
                end

                % Get transformed density evaluated on stress grid
                rho = obj.interpolateRhoToStressGrid(b);

                % Get grid sizes
                g_s = obj.grid.grids{b}.gridGroups{2}{1};
                m = g_s.logic.m;
                hxi = 1/(m(1)-1);
                heta = 1/(m(2)-1);
                h = min(hxi,heta);

                % Compute speed in a few different directions
                theta_range = linspace(0,pi,20);
                v_max = elastic.maxAnisotropicWaveSpeed(rho, C, theta_range);
                dt(b) = cfl/v_max*h;
            end
            dt = min(dt);
            k = dt;
        end

        % Interpolate transformed rho to stress grid
        function rho_s = interpolateRhoToStressGrid(obj, blockId)
            rho = diag(obj.diffOp.diffOps{blockId}.refObj.RHO{1});

            g_u = obj.grid.grids{blockId}.gridGroups{1}{1};
            g_s = obj.grid.grids{blockId}.gridGroups{2}{1};

            rhoFun = scatteredInterpolant(g_u.points(), full(rho), 'linear', 'linear');
            rho_s = rhoFun(g_s.points());
        end

        function r = getTimeSnapshot(obj, ts)
            if ts == 0
                r.t = 0;
                r.v = obj.v0;
                r.vt = obj.v0t;
                return
            end
            r.t = ts.t;
            r.v = ts.getV();
            r.vt = ts.getVt();
        end

        % Sets up movie recording to a given file.
        %     saveFrame is a function_handle with no inputs that records the current state
        %               as a frame in the moive.
        function saveFrame = setupMov(obj, file)
            error('not implemented');
        end

        % Sets up a plot of the discretisation
        %     update is a function_handle accepting a timestepper that updates the plot to the
        %            state of the timestepper
        function [update,figure_handle] = setupPlot(obj, type)

            subGrid = 1;
            G = obj.G{subGrid};
            U1 = obj.U{subGrid}{1};
            U2 = obj.U{subGrid}{2};

            E1 = (U1'*G')';
            E2 = (U2'*G')';

            g = obj.grid;
            figure_handle = figure();
            v0 = obj.v0t;
            % v0 = obj.v0;
            v1 = E1'*v0;
            v2 = E2'*v0;

            DOI_IDs = obj.superGrid.DOI_IDs;
            nDOIGrids = numel(DOI_IDs);
            g_DOI = g.grids(DOI_IDs);
            conn = g.connections(DOI_IDs, DOI_IDs);
            g_DOI = multiblock.Grid(g_DOI, conn);

            % Loop through grids and compress them
            factor = 100;
            for i = 1:g.nBlocks

                if ismember(i, obj.superGrid.W_IDs)
                    X = g.grids{i}.gridGroups{1}{subGrid}.coords(:,1);
                    xr = max(X);
                    X = xr - (xr-X)/factor;
                    g.grids{i}.gridGroups{1}{subGrid}.coords(:,1) = X;
                end

                if ismember(i, obj.superGrid.N_IDs)
                    Y = g.grids{i}.gridGroups{1}{subGrid}.coords(:,2);
                    yl = min(Y);
                    Y = yl + (Y-yl)/factor;
                    g.grids{i}.gridGroups{1}{subGrid}.coords(:,2) = Y;
                end

                if ismember(i, obj.superGrid.S_IDs)
                    Y = g.grids{i}.gridGroups{1}{subGrid}.coords(:,2);
                    yr = max(Y);
                    Y = yr - (yr-Y)/factor;
                    g.grids{i}.gridGroups{1}{subGrid}.coords(:,2) = Y;
                end

                if ismember(i, obj.superGrid.E_IDs)
                    X = g.grids{i}.gridGroups{1}{subGrid}.coords(:,1);
                    xl = min(X);
                    X = xl + (X-xl)/factor;
                    g.grids{i}.gridGroups{1}{subGrid}.coords(:,1) = X;
                end

            end

            % u1, full domain
            h1 = subplot(2,1,1);
            elastic.cmap;
            Sur1 = multiblock.StaggeredSurface(g, v1);
            xlabel('x')
            ylabel('y')
            shading interp
            colorbar
            xlims1 = xlim;
            ylims1 = ylim;
            axis equal
            a = gca;

            % u2, full domain
            h2 = subplot(2,1,2);
            elastic.cmap;
            Sur2 = multiblock.StaggeredSurface(g, v2);
            xlabel('x')
            ylabel('y')
            shading interp
            colorbar
            xlims2 = xlim;
            ylims2 = ylim;
            axis equal;


            function update_fun(r,E1,E2,xlims1,ylims1,xlims2,ylims2)
                t = r.t;
                % v1 = E1'*r.v;
                % v2 = E2'*r.v;
                v1 = E1'*r.vt;
                v2 = E2'*r.vt;
                if ishandle(a)
                    title(a,sprintf('T = %.3f',t))
                end

                Sur1.ZData = v1;
                Sur1.CData = v1;
                Sur2.ZData = v2;
                Sur2.CData = v2;
                xlim(h1, xlims1);
                ylim(h1, ylims1);
                xlim(h2, xlims2);
                ylim(h2, ylims2);

                cmax = 1;
                caxis(h1, [-cmax, cmax]);
                caxis(h2, [-cmax, cmax]);
            end
            update = @(r)update_fun(r,E1,E2,xlims1,ylims1,xlims2,ylims2);

        end

        % Compare two functions u and v in the discrete l2 norm.
        function e = compareSolutions(obj, u, v)
            H = obj.diffOp.H;

            evec = u - v;
            e = sqrt( evec'*H*evec );
        end

        % Compare the grid function u to the analytical function g in the discrete l2 norm.
        function e = compareSolutionsAnalytical(obj, u, g, t)
            gr = obj.grid;
            v = grid.evalOnStaggered(gr, @(x,y)g(t,x,y));
            e = obj.compareSolutions(u, v);
        end

    end

    methods(Static)

    end
end