classdef poroelasticDiscrTotal < noname.Discretization
    properties
        name         = 'poroelastic 2D'
    ...
    methods
        ...
        %       - types: e.g. {'d', 'd', 'd','d'; 'free','dirichlet', 't', 't'; 'd','n', 'd', 'd'}
        %       - funcs: cell array of function handles for boundary data, e.g
        %                {g_u1W, g_u1E, g_u1S, g_u1N;...
        %                 g_u2W, g_U2E, g_u2S, g_u2N;...
        %                 g_pW,  g_pE,  g_pS,  g_pN}
        %
        % The ordering is {u1W, u1E, u1S, u1N; ...
        %                  u2W, u2E, u2S, u2N; ...
        %                   pW,  pE,  pS,  pN}; 

        function obj = poroelasticDiscrTotal(m, order, xlim, ylim, coeff_fun, F, boundary_data, opSet, BCstrength)
            ...
            rho_fun = [];
            diffOpElastic = scheme.Elastic2dVariable(g, order, lambda_fun, mu_fun, rho_fun, opSet);
            diffOpHeat = scheme.Heat2dVariable(g ,order, kappa_fun, opSet);
            H = diffOpHeat.H;
            ...
            % Divergence operator
            E = diffOpElastic.E;
            Dx = diffOpElastic.D1{1};
            Dy = diffOpElastic.D1{2};
            Div = Dx*E{1}' + Dy*E{2}';
            ...
            BC_p_S = BC{3,3};
            BC_p_N = BC{3,4};
            [closure_W_heat, penalty_W_heat] = diffOpHeat.boundary_condition('w',BC_p_W,true,BCstrength);
            [closure_E_heat, penalty_E_heat] = diffOpHeat.boundary_condition('e',BC_p_E,true,BCstrength);
            ...
            Laplace_kappa = Laplace_kappa + closure_W_heat + closure_E_heat + ...
                                            closure_S_heat + closure_N_heat;
            ...
            penalty_W_heat = M*penalty_W_heat;
            penalty_E_heat = M*penalty_E_heat;
            penalty_S_heat = M*penalty_S_heat;
            penalty_N_heat = M*penalty_N_heat;
            F_b.W = penalty_W_heat;
            F_b.E = penalty_E_heat;
            F_b.S = penalty_S_heat;
            F_b.N = penalty_N_heat;
            %-------------------------------------

            %--- Mechanical equilibrium equation ----
            BC_displ_W = {BC{1,1}, BC{2,1}};
            BC_displ_E = {BC{1,2}, BC{2,2}};
            BC_displ_S = {BC{1,3}, BC{2,3}};
            BC_displ_N = {BC{1,4}, BC{2,4}};

            [closure_W_elastic, penalty_W_elastic] = ...
                                diffOpElastic.boundary_condition('w',BC_displ_W);
            [closure_E_elastic, penalty_E_elastic] = ...
                                diffOpElastic.boundary_condition('e',BC_displ_E);
            [closure_S_elastic, penalty_S_elastic] = ...
                                diffOpElastic.boundary_condition('s',BC_displ_S);
            [closure_N_elastic, penalty_N_elastic] = ...
                                diffOpElastic.boundary_condition('n',BC_displ_N);

            D_elastic = D_elastic + closure_W_elastic + closure_E_elastic + ...
                                    closure_S_elastic + closure_N_elastic;
            ...
            M_bu.W = [penalty_W_elastic{1}, penalty_W_elastic{2}];
            M_bu.E = [penalty_E_elastic{1}, penalty_E_elastic{2}];

            % Matrix for p
            M_p = -(E{1}*Dx + E{2}*Dy)*alpha;

            % Operators for special BC
            Hi = diffOpElastic.Hi;
            eW = diffOpElastic.get_boundary_operator('e', 'w');
            eE = diffOpElastic.get_boundary_operator('e', 'e');
            ...
            case {'T','t','Traction','traction'};
                M_p = M_p + penalty_W_elastic{k}*n*eW'*alpha; 
            end
            % --- East --- -%
            n = 1;
            switch BC_displ_E{k}
            case {'T','t','Traction','traction'};
                M_p = M_p + penalty_E_elastic{k}*n*eE'*alpha;
            end
            % ---------------- %
            ...
            % Pressure BC
            j = 1; % Boundary number
            H_gamma = diffOpElastic.H_boundary{j};
            % ----- West ---- %
            n = -1;
            switch BC_p_W
            case {'D','d','Dirichlet','dirichlet'}
                M_p = M_p + n*E{j}*Hi*alpha*eW*H_gamma*eW';
                M_bp.W =  - n*E{j}*Hi*alpha*eW*H_gamma;
            otherwise
                M_bp.W = 0*E{j}*Hi*alpha*eW*H_gamma;
            end
            ...
            switch BC_p_E
            case {'D','d','Dirichlet','dirichlet'}
                M_p = M_p + n*E{j}*Hi*alpha*eE*H_gamma*eE';
                M_bp.E =  - n*E{j}*Hi*alpha*eE*H_gamma;
            otherwise
                M_bp.E =  0*E{j}*Hi*alpha*eE*H_gamma;    
            end
            ...
            switch BC_p_S
            case {'D','d','Dirichlet','dirichlet'}
                M_p = M_p + n*E{j}*Hi*alpha*eS*H_gamma*eS';
                M_bp.S =  - n*E{j}*Hi*alpha*eS*H_gamma;
            otherwise
            ...
            switch BC_p_N
            case {'D','d','Dirichlet','dirichlet'}
                M_p = M_p + n*E{j}*Hi*alpha*eN*H_gamma*eN';
                M_bp.N =  - n*E{j}*Hi*alpha*eN*H_gamma;
            otherwise
            ...
            penalty_W = {E_u*penalty_W_elastic{1}; E_u*penalty_W_elastic{2}; ...
                        E_p*penalty_W_heat + E_u*M_bp.W};
            penalty_E = {E_u*penalty_E_elastic{1}; E_u*penalty_E_elastic{2}; ...
                        E_p*penalty_E_heat + E_u*M_bp.E};
            ...
            % West boundary
            for k = 1:3
                if ~isempty(bc_data{k,1})
                    data_funcs{1+k,1} = @(t) penalty_W{k}*bc_data{k,1}(eW'*x, eW'*y, t);
                end
            end
            % West boundary
            for k = 1:3
                if ~isempty(bc_data{k,2})
                    data_funcs{1+k,2} = @(t) penalty_E{k}*bc_data{k,2}(eE'*x, eE'*y, t);
                end
            end
            % South boundary
            for k = 1:3
                if ~isempty(bc_data{k,3})
                    data_funcs{1+k,3} = @(t) penalty_S{k}*bc_data{k,3}(eS'*x, eS'*y, t);
                end
            end
            % North boundary
            for k = 1:3
                if ~isempty(bc_data{k,4})
                    data_funcs{1+k,4} = @(t) penalty_N{k}*bc_data{k,4}(eN'*x, eN'*y, t);
                end
            end
        ...
        function [ts, N] = getTimestepper(obj,method,time_align,t_start,cfl) %% ???
            default_arg('method','sbp');
            default_arg('time_align',[]);
            default_arg('t_start',0)
            default_arg('cfl',2);
            switch method
                case 'sbp'
                    ...
                    if ~isempty(time_align)
                        [k, N] = alignedTimestep(k, time_align-t_start);
                    end
