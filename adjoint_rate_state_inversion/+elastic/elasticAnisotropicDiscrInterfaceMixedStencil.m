classdef elasticAnisotropicDiscrInterfaceMixedStencil < noname.Discretization
    properties
        name         = 'elastic 2D multiblock with point source, mixed stencil'
    ...
    methods
        ...
        %               g = {{g1x, g1y}, {g2x, g2y}}
        function obj = elasticAnisotropicDiscrInterfaceMixedStencil(m, order, xlim, ylim, C_D1, C_D2, rho, F,...
                                             boundary_data, point_sources, opSet)
            default_arg('opSet',[]);
            ...
            for i = 1:nBlocks
                if iscell(rho)
                    doParam{i} = {rho{i}, C_D1{i}, C_D2{i}, opSet};
                elseif isa(rho, 'function_handle')
                    doParam{i} = {rho, C_D1, C_D2, opSet};
                else
                    error('Inconsistent format for C and rho.');
                end
            end

            diffOp = multiblock.DiffOp(@scheme.Elastic2dVariableAnisotropicMixedStencil, g, order, doParam);
            D = diffOp.D;
            ...
            for b = 1:nBlocks
                ...
                for i = 1:dim
                    for j = 1:dim
                        for k = 1:dim
                            for l = 1:dim
                                C{i,j,k,l}{b,b} = diffOp.diffOps{b}.C_D1{i,j,k,l} ...
                                                + diffOp.diffOps{b}.C_D2{i,j,k,l};
                            end
                        end
                    end
                end
