...
methods
    ...
    function obj = setFaultTraction(obj)
        ...
        case 'erickson2022'
            ...
            V_star_from_tau_l = @(i_t, tau_l, Psi) -1./((eta + F_V(i_t))).*(tau_l + G_V(i_t).*Psi ...
            + F_V_Psi(i_t).*V_dagger(i_t).*delta_Psi(i_t) + G_V_Psi .* Psi_dagger(i_t) .* delta_Psi(i_t) + F_V_V(i_t) .* V_dagger(i_t) .* delta_V(i_t) ...
            + G_V_V(i_t) .* Psi_dagger(i_t) .* delta_V(i_t) + eps_a .* (G_V_a .* Psi_dagger(i_t) + F_V_a(i_t) .* V_dagger(i_t)));
    ...
    function obj = setStateEvolution(obj)
        ...
        case 'erickson2022'
            ...
            + (F_V_Psi(i_t) .* V_dagger(i_t) .* delta_V(i_t) + G_V_Psi .* Psi_dagger(i_t) .* delta_V(i_t)) ...
            + eps_a .* (G_Psi_a .* Psi_dagger(i_t) + F_Psi_a(i_t) .* V_dagger(i_t)));
        end
    end
