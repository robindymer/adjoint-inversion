classdef SupergridLayerNE < multiblock.DefCurvilinear
    properties
        p
        m
        ratio
    ...
    methods
        function obj = SupergridLayerNE(p, h0x, h0y, m, ratio)
            default_arg('m',51);
            ...
            boundaryGroups.farfield = multiblock.BoundaryGroup({{1,'e'}, {1,'n'}});

            boundaryGroups.E = multiblock.BoundaryGroup({{1,'e'}});
            boundaryGroups.N = multiblock.BoundaryGroup({{1,'n'}});
