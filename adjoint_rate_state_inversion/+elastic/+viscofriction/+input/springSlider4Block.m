function [domain, friction, preStress, material, mms, flowLaw, plotting, tectonic, timeStepper, output] = springSlider4Block()
	...
	% ----- Friction parameters ------------------
	% Both a and b constant
	friction.a = 0.02; 			% Direct effect [ ]
	friction.b = 0.025;			% State evolution effect [ ]
	friction.V0 = 1e-6;			% Ref velocity [ m/s ]
	friction.f0 = 0.6;			% Ref friction coeff for steady sliding [ ]
	friction.L = 0.05;			% State evolution distance
	...
	rho_ref = rho(0,0);
	vs_ref = vs(0,0);
	mu_ref = rho_ref*vs_ref^2;
	friction.eta = mu_ref/(2*vs_ref);	% Radiation damping
	%----------------------------------------------
	...
	hstar = mu_ref*friction.L/((friction.b-friction.a)*preStress.sigma0(0,0));
	qsProcessZone = mu_ref*friction.L/(friction.b*preStress.sigma0(0,0));
	nucleationZoneSize = 2/pi*mu_ref*friction.b*friction.L/((friction.b-friction.a)^2*preStress.sigma0(0,0) );
	fprintf('hstar: %3.1f km \n', hstar/1e3);
	...
	tectonic.u0 = 0*1*H/mu_ref*friction.f0*preStress.sigma0(0,0);
	...
	% times = (0:1e3:1e5)*oneYear;

	% fieldTimes = (0:1e3:1e5)*oneYear;
...
function etaInv = flowLawFun(sigma)
	...
	% etaInv = 1e15*A*exp(-Q/(R*T))*tauBar.^(n-1);
