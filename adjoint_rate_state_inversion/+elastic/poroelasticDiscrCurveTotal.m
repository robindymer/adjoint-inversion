classdef poroelasticDiscrCurveTotal < noname.Discretization
    properties
        name         = 'poroelastic curvilinear 2D'
    ...
    methods
        ...
        function obj = poroelasticDiscrCurveTotal(g, order, coeff_fun, F, boundary_data, opSet, BCstrength)
            ...
            case {'N','n','Neumann','neumann'}
                penalty_W_heat = penalty_W_heat*kappaW;
            end
            switch BC_p_E
            case {'N','n','Neumann','neumann'}
                penalty_E_heat = penalty_E_heat*kappaE;
            end
            switch BC_p_S
            case {'N','n','Neumann','neumann'}
            ...
            case {'D','d','Dirichlet','dirichlet'}
                for i = 1:dim
                    M_p = M_p + n*E{i}*Ji*Hi*inv(beta{j})*J*b{i,j}*alpha*eW*H_gamma*eW';
                    M_bp.W = M_bp.W - n*E{i}*Ji*Hi*inv(beta{j})*J*b{i,j}*alpha*eW*H_gamma;
                end
            otherwise
            ...
            case {'D','d','Dirichlet','dirichlet'}
                for i = 1:dim
                    M_p = M_p + n*E{i}*Ji*Hi*inv(beta{j})*J*b{i,j}*alpha*eE*H_gamma*eE';
                    M_bp.E = M_bp.E - n*E{i}*Ji*Hi*inv(beta{j})*J*b{i,j}*alpha*eE*H_gamma;
                end
            otherwise
