...
void D2y_2nd(double *u_x, double *u, double *b, double dx, int Nx, int Ny)
{
    int i, j, bp, idx;
...
void D2y_4th(double *u_x, double *u, double *b, double dx, int Nx, int Ny)
{
    int i, j, bp;
...
void D2y_6th(double *u_x, double *u, double *b, double dx, int Nx, int Ny)
{
    int i, j, bp;
    ...
    #pragma omp parallel for private(i,j,im3,im2,im1,i0,ip1,ip2,ip3,wm3,wm2,wm1,w0,wp1,wp2,wp3,wp2_m1,wp3_m2,wp3_m1)
    for (j=bp; j<Nx-bp; j++){
        i = bp;
        i0 = j*Ny + i;
        im3 = i0-3;
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    if (order == 2){
        D2y_2nd(out, u, b, dx, Nx, Ny);
    }
    else if (order == 4){
        D2y_4th(out, u, b, dx, Nx, Ny);
    }
    else if (order == 6){
        D2y_6th(out, u, b, dx, Nx, Ny);
    }
}