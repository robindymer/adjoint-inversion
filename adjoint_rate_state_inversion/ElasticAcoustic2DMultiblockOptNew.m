...
classdef ElasticAcoustic2DMultiblockOptNew < adjopt.AdjointOptimization
	...
	methods
		...
		function obj = ElasticAcoustic2DMultiblockOptNew(pars, inversionPars, order, k)
			...
			forwardDiscr = elastic.elasticAcousticDiscr(domain, order, pars, bc, [], sources);
			...
			if ismember('lambda', ip) || ismember('mu',ip)
				...
				% Correct for traction BC
				intfBG = domain.elastic.def.boundaryGroups.(domain.elastic.interfaceGroup);
				bc1.boundary = intfBG;
				bc1.type = {1, 't'};
				bc2.boundary = intfBG;
				bc2.type = {2, 't'};
				bcExtended = bc.elastic;
				bcExtended{end+1} = bc1;
				bcExtended{end+1} = bc2;
			end
			if ismember('lambda', ip)
				m = length(pars.elastic.lambda);
				lambda = 0*pars.elastic.lambda;
				mu = 0*lambda;
				rho = 0*lambda + 1;
				mu_split = forwardDiscr.g.elastic.splitFunc(mu);
				rho_split = forwardDiscr.g.elastic.splitFunc(rho);

				dL_dLambda = cell(m);
				for i = 1:m
					lambda(i) = 1;
					lambda_split = forwardDiscr.g.elastic.splitFunc(lambda);
					doParam = cell(1, forwardDiscr.g.elastic.nBlocks());
					for j = 1:nBlocks
						doParam{j} = {lambda_split{j}, mu_split{j}, rho_split{j}};
					end
					diffOp = multiblock.DiffOp(@Elastic2dVariableShell, forwardDiscr.g.elastic, order, doParam);
					closure = scheme.bcSetup(diffOp, bcExtended);
            		diffOp.D = diffOp.D + closure;
					dL_dLambda{i} = -diffOp.D;
					lambda(i) = 0;
				end
				clear diffOp;
				obj.dLdp.lambda = dL_dLambda;
				clear dL_dLambda;
			end
			if ismember('mu', ip)
				m = length(pars.elastic.mu);
				mu = 0*pars.elastic.mu;
				lambda = 0*mu;
				rho = 0*mu + 1;
				lambda_split = forwardDiscr.g.elastic.splitFunc(lambda);
				rho_split = forwardDiscr.g.elastic.splitFunc(rho);
		...
		function discr = createElasticAcousticDiscr(obj, pars)
			...
			discr = elastic.elasticAcousticDiscr(domain, order, pars, bc, [], sources);
		...
		function grad = gradientFormula(obj)
			...
			for i = 1:numel(obj.inversionPars)
				...
				case {'mu', 'lambda'}
					...
					dLdp = obj.dLdp.(obj.inversionPars{i});
					...
					for q = 1:nPar
						grad{i}(q) = grad{i}(q) - obj.timeIntegration(H_kron_row_vec*(psi.*(dLdp{q}*u)));
					end
