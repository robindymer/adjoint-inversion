classdef Elastic2dVariableShell < handle
...
% is useful if we have periodic BC in one direction.
    ...
    methods
        ...
        function obj = Elastic2dVariableShell(g ,order, lambda, mu, rho, opSet)
            ...
            mu_mat = grid.funcToMatrix(g, mu);
            lambda_mat = grid.funcToMatrix(g, lambda);
            [muI, muJ] = find(mu_mat);
            [lambdaI, lambdaJ] = find(lambda_mat);

            if ~isempty(muI) || ~isempty(lambdaI)
        ...
        function [closure, penalty] = boundary_condition(obj, boundary, bc, tuning)
            ...
            if max(max(e'*obj.LAMBDA*e + e'*obj.MU*e)) == 0
                dim = obj.dim;
                m_tot = obj.grid.N();
                [~, nBoundary] = size(e);

                closure = sparse(dim*m_tot, dim*m_tot);
                penalty = sparse(dim*m_tot, dim*nBoundary);
                return;
            end
